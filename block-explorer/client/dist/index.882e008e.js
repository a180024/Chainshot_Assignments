...
document.getElementById("public-address").addEventListener("input", async ({ target: { value  }  })=>{
    ...
    const response = await _axiosDefault.default.get(`${server}/balance/${value}`);
    document.getElementById("balance").innerHTML = Math.round(response.data.balance * 10000) / 10000;
    console.log(response.data.isContract);
    let html = "";
    if (response.data.isContract) html = "<p> Contract: True</p>";
    else html = "<p> Contract: False</p>";
    document.getElementById("is-contract").innerHTML = html;
});
document.getElementById("get-latest").addEventListener("click", async ()=>{
    const response = await _axiosDefault.default.get(`${server}/block`);
    const latestBlock = response.data.block;
    let html = "";
    html += "<p> Latest Block Hash: " + latestBlock.hash + "</p>" + "<p> Miner: " + latestBlock.miner + "</p>";
    document.getElementById("block-info").innerHTML = html;
});

},{"./index.scss":"fgJeu","axios":"jo6P5","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"fgJeu":[function() {},{}],"jo6P5":[function(require,module,exports) {
...
},{}]},["9MTMI","frMjV"], "frMjV", "parcelRequirebbbf")
